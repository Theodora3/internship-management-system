...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		 http
	        .authorizeHttpRequests(authz -> authz
	            ...
	            .requestMatchers("/professor_dashboard").permitAll()
	            .anyRequest().authenticated()
	        )
	        .formLogin(form -> form
	        	    .loginPage("/login")
	        	    .loginProcessingUrl("/login")
	        	    .successHandler(customLoginSuccessHandler())  // <=== ΠΟΛΥ ΣΗΜΑΝΤΙΚΟ!
	        	    .permitAll()
	        )
    ...
    public PasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder();  // Χρησιμοποιούμε τον BCryptPasswordEncoder για την ασφάλεια των κωδικών
    }
}
